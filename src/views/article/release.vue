...
export default {
        ...
        new Image({
          ...
          uploadRequest(file) {
            ...
            return uploadImage(fd).then(res => {
              // console.log(res)
              // 这个 return 是返回最后的结果
              return 'http://172.19.10.109:3000' + res.data
            })
  ...
  methods: {
    ...
    async getArticle() {
      ...
      const res = await seArticle({ id: this.$route.query.id })
      const arr = res.data.map(item => ({ id: item.id, title: item.title, introduce: item.introduce, cover: item.cover, category: item.category, section: item.section }))
      this.formData = arr[0]
      console.log(arr[0].cover)
      arr[0].cover.forEach(item => {
        if (item.imgUrl.indexOf('http://172.19.10.109:3000')) {
          this.imgUrlList.push({ url: 'http://172.19.10.109:3000' + item.imgUrl })
        } else {
